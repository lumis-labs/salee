...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path == "/":
            ...
            self._send(200, {"ok": True, "agent_name": self.settings.agent_name, "agent_full_name": self.settings.agent_full_name, "stripe_mode": self.worker.payment_mode, "revenue_ready": self.worker.revenue_ready, "missing_revenue_config": self.worker.missing_revenue_config, "missing_operational_config": self.settings.missing_operational_config, **self.worker.store.status()})
